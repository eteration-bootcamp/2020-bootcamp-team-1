import React from "react";

const TimerSvg = () => {
  return (
    <svg
      width="15"
      height="15"
      version="1.1"
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink"
      x="0px"
      y="0px"
      viewBox="0 0 512 512"
      fill="#888"
      style={{ marginRight: "6px", position: "relative", top: "-1.5px" }}
      xmlSpace="preserve"
    >
      <g>
        <g>
          <g>
            <circle cx="256" cy="30" r="30" />
            <path
              d="M255,275c11.046,0,20-8.954,20-20c0-5.523-2.238-10.523-5.857-14.142l-177.5-177.5L63.357,91.642l177.5,177.5
        C244.477,272.761,249.477,275,255,275z"
            />
            <path
              d="M453.344,92.921C419.49,51.998,373.56,22.55,322.716,8.82C324.844,15.505,326,22.619,326,30
        c0,7.156-1.084,14.063-3.089,20.57C410.603,79.116,472,161.905,472,256c0,119.103-96.897,216-216,216S40,375.103,40,256
        c0-37.107,9.305-72.772,26.787-104.36l-29.25-29.25C13.085,162.203,0,208.089,0,256c0,68.38,26.629,132.667,74.98,181.02
        C123.333,485.371,187.62,512,256,512s132.667-26.629,181.02-74.98C485.371,388.667,512,324.38,512,256
        C512,196.562,491.169,138.646,453.344,92.921z"
            />
          </g>
        </g>
      </g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
    </svg>
  );
};

export default TimerSvg;
